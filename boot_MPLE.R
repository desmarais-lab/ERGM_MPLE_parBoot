...
boot.MPLE<- function(ergm.formula, boot.rep=500, mcmc.interval=1024, mcmc.burnin=16*1024, number.cores=1){
  ...
  # extract network from formula
  net <- terms(ergm.formula)[[2]]
  ...
  boot.coef<- foreach(i= 1:boot.rep, .combine=cbind)%dopar%{
  
  ### foreach requires to load ergm package again  
  library(ergm)
  # extract covariates
  ...
  colnames(degeneracy.test)<- c("sim > obs", "obs", "sim 25%", "sim 50%", "sim 75%")
  ...
  for(j in 1:number.parameters){
    ...
    degeneracy.test[j,1]<- length(boot.coef[j+number.parameters, ][boot.coef[j+number.parameters, ]> observed.network.stats])/boot.rep
